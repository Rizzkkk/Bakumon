...
9. **[partial] Domain: `bakumon.net`.** Supplied 2026-09-25. Already wired in:
   `VITE_SITE_URL` in `apps/web/.env` (the canonical link and every absolute URL in the
   structured data), the absolute `og:url`/`og:image` in `index.html`, the `Sitemap:` line
   in `robots.txt`, and 1,844 URLs in `sitemap.xml`. **Still outstanding**: DNS access
   itself, the Hostinger VPS credentials (ADR 0005 landed on the VPS), and adding
   `https://bakumon.net` to the production `CORS_ORIGINS` - `.env.example` shows the value,
   but the deployed `.env` is the one that matters and must be *appended* to, never
   line-replaced.
...
16. **[done] Per-page `<title>`, meta description, canonical and structured data.**
    `apps/web/src/hooks/useHead.js`, set inside an effect so prerendering stays possible
    (ADR 0008). The earlier note here said per-page meta descriptions had "the same
    client-rendered ceiling as `og:` tags"; that was wrong, and the distinction matters.
    Social scrapers do not run JavaScript, so `og:` really is stuck at the site-wide
    values. **Google does render JavaScript**, so `<title>`, `<meta name="description">`,
    `<link rel="canonical">` and JSON-LD injected at runtime are all read.

    Measured 2026-09-25 in a real browser across nine routes: nine distinct titles, every
    one under 60 characters; a canonical on every indexable route, with `?q=`/`?bucket=`
    collapsing to the bare path and `?page=` self-canonical; `WebSite`+`SearchAction` on
    `/`, `BreadcrumbList` on the five wiki routes, `ItemList` on both indexes and `Article`
    on both detail pages. The 404 carries `noindex` and deliberately no canonical.
...
19. **[done] robots.txt** and **sitemap.xml**. Generated from the database by
    `SITE_URL=https://bakumon.net npm run sitemap`, never hand-maintained: 1,844 URLs
    (6 static, 904 Pokemon, 934 items) plus the `Sitemap:` line written into `robots.txt`.
    Re-run it after any workbook re-export, or the sitemap names pages that no longer
    exist.
